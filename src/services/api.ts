...
export const getTelegramMessageById = async (
	id: string,
...
export const searchTelegramMessages = async (
	query: string,
): Promise<TelegramResponse> => {
...
export const updateMessageCategory = async (
	...
	try {
		...
		console.log(
			`Updating category for message ID: ${messageId} to ${category}`,
		);
		const response = await api.put(`/telegram/messages/${messageId}/category`, {
			category,
		});
		return response.status === 200;
	} catch (error) {
