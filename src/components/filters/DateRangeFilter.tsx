			...
			<Popover open={open} onOpenChange={setOpen}>
				<PopoverTrigger asChild>
					<Button
						...
						<CalendarIcon
							size={12}
							className="sm:size-[14px] mx-auto opacity-80 -mr-1 -ml-0.5 sm:ml-0.5"
						/>
				...
				<PopoverContent
					align="end"
					onOpenAutoFocus={(e) => {
						// On mobile, Radix will autofocus the first focusable element (our Input),
						// which opens the on-screen keyboard immediately. Prevent that.
								...
								<div>
									<div className="mt-2 flex items-center justify-center gap-2">
										...
										<Input
											...
											inputMode="numeric"
											className="h-9 text-xs w-24 px-2"
										/>
									</div>
									{fromDraftError && (
					...
					<UiCalendar
						...
						classNames={
							isMobile
								? {
										months:
											'flex flex-col space-y-2 sm:flex-row sm:space-x-4 sm:space-y-0',
										month: 'space-y-2',
										head_cell:
											'text-muted-foreground rounded-md w-7 font-normal text-[0.7rem]',
										// Keep the default "selected" rounding logic from `ui/calendar.tsx`
										// while shrinking the calendar footprint on small screens.
