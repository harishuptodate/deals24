	...
	return (
		<Carousel
			...
			<div className="relative">
							...
							<CarouselItem
								...
								<div className="h-[320px] w-full">
									<CustomDealCard
										...
										onCategoryUpdate={
											isAuthenticated() ? onCategoryUpdate : undefined
										}
									/>
								</div>
							</CarouselItem>
				...
				{/* Navigation buttons */}
				<CarouselPrevious className="left-2 bg-white/90 hover:bg-white dark:bg-apple-darkGray dark:hover:bg-gray-700 shadow-lg" />
				<CarouselNext className="right-2 bg-white/90 hover:bg-white dark:bg-apple-darkGray dark:hover:bg-gray-700 shadow-lg" />
			</div>
...
const CustomDealCard = ({
	...
	const [isCategoryPasswordDialogOpen, setIsCategoryPasswordDialogOpen] =
		useState(false);
	...
	const { isSaved, isSharing, handleToggleWishlist, handleShare } =
		useDealCardActions({
			id: formattedDeal.id,
			title,
			description,
	...
	const links = extractFirstLink(description)
					...
					{extractFirstLink(description) && (
						<div className="mt-auto flex justify-center">
							<a
								...
								className="inline-block w-full max-w-[300px] text-center px-4 py-2 text-sm font-medium text-white bg-gradient-to-b from-apple-darkGray to-indigo-950 rounded-full transition-all duration-300 hover:shadow-lg hover:shadow-apple-darkGray/20 hover:scale-105">
								Buy Now
							</a>
						</div>
					)}
